...
//find i for the addition
  ...
  database.ref('Log').push({"date": moment().format('DD-MM-YYYY, HH:mm:ss'), "amount": del * mult, "product": products[i], "user":currentUser, "deviceInfo": window.navigator.userAgent});
  database.ref('users/' + currentUser).once('value').then(function(snapshot){
